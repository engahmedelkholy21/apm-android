...
public class Mosta7akatDa2neenActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mosta7akat_da2neen);
    ...
    private void setupCustomerAutoComplete() {
        try {
            String query = "SELECT DISTINCT mosta7akat_da2neen_cst_name FROM mosta7akat_da2neen_table ORDER BY mosta7akat_da2neen_cst_name";
    ...
    private void fetchData() {
        ...
        executor.execute(() -> {
            try {
                String query = "SELECT * FROM mosta7akat_da2neen_table " +
                        "WHERE mosta7akat_da2neen_cst_name = ? " +
                        "AND mosta7akat_da2neen_date BETWEEN ? AND ? " +
                        "ORDER BY mosta7akat_da2neen_date DESC";
    ...
    private void processResultSet(ResultSet rs) throws Exception {
        ...
        while (rs.next()) {
            ...
            totalDue += rs.getDouble("mosta7akat_da2neen_matloob_after");
            totalPaid += rs.getDouble("mosta7akat_da2neen_dof3ah_amount");
            totalRemaining += rs.getDouble("mosta7akat_da2neen_matloob_after");
        }

        // Add summary row with yellow background
        if (!rows.isEmpty()) {
            TableRow summaryRow = createSummaryRow(totalRemaining);
            rows.add(summaryRow);
        }
    ...
    private TableRow createTableRow(ResultSet rs, int index) throws Exception {
        ...
        row.addView(createCell(String.valueOf(rs.getInt("mosta7akat_da2neen_id")), 50));
        row.addView(createCell(rs.getString("mosta7akat_da2neen_type"), 65));
        row.addView(createCell(rs.getString("mosta7akat_da2neen_date")));
        row.addView(createCell(rs.getString("mosta7akat_da2neen_notes"), 400));

        TextView beforeCell = createCell(String.format(Locale.US, "%.2f", rs.getDouble("mosta7akat_da2neen_matloob_before")), 120);
        ...
        TextView dofaCell = createCell(String.format(Locale.US, "%.2f", rs.getDouble("mosta7akat_da2neen_dof3ah_amount")));
        dofaCell.setBackgroundColor(0xFFE0FFE0); // Light green
        row.addView(dofaCell);

        row.addView(createCell(String.format(Locale.US, "%.2f", rs.getDouble("mosta7akat_da2neen_matloob_after")), 120));
        row.addView(createCell(rs.getString("mosta7akat_da2neen_user"), 120));

        // Store fatora_number for click handler
        String fatoraNumber = rs.getString("fatora_number");
        row.addView(createCell(fatoraNumber, 60));

        // Make row clickable to show purchase invoice
        if (fatoraNumber != null && !fatoraNumber.equals("0")) {
            row.setOnClickListener(v -> showPurchaseInvoice(fatoraNumber));
        }
    ...
    private TableRow createSummaryRow(double totalRemaining) {
        TableRow row = new TableRow(this);
        row.setPadding(8, 8, 8, 8);
        row.setBackgroundColor(0xFFFFFF00); // Yellow background

        // Add empty cells for alignment
        for (int i = 0; i < 7; i++) {
            row.addView(createCell("", 0));
        }

        // Add total remaining amount
        row.addView(createCell(String.format(Locale.US, "%.2f", totalRemaining), 120));

        // Add empty cells for remaining columns
        row.addView(createCell("", 120));
        row.addView(createCell("", 60));

        return row;
    }

    private void showPurchaseInvoice(String invoiceNumber) {
        if (invoiceNumber != null && !invoiceNumber.equals("0")) {
            // TODO: Implement showing purchase invoice details
            Toast.makeText(this, "عرض تفاصيل فاتورة المشتريات رقم: " + invoiceNumber, Toast.LENGTH_SHORT).show();
        }
    }
