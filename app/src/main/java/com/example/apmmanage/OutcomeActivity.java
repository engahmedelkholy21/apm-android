...
public class OutcomeActivity extends AppCompatActivity {
            ...
            try {
                ...
                if (project.isEmpty()) {
                    if (fromDate.equals(toDate)) {
                        query = "SELECT * FROM outcome_table WHERE outcome_date = ? AND outcome_far3 = ?";
                        hasData = executeQuery(query, new String[]{fromDate, branch});
                    } else {
                        query = "SELECT * FROM outcome_table WHERE outcome_date BETWEEN ? AND ? AND outcome_far3 = ?";
                        hasData = executeQuery(query, new String[]{fromDate, toDate, branch});
                    }
                } else {
                    query = "SELECT * FROM outcome_table WHERE outcome_date BETWEEN ? AND ? AND outcome_esm_elmashroo3 = ? AND outcome_far3 = ?";
                    hasData = executeQuery(query, new String[]{fromDate, toDate, project, branch});

                    if (!hasData) {
                        query = "SELECT * FROM outcome_table WHERE outcome_date BETWEEN ? AND ? AND outcome_user = ? AND outcome_far3 = ?";
                        hasData = executeQuery(query, new String[]{fromDate, toDate, project, branch});
                        if (hasData) {
                            mainHandler.post(() ->
                ...
                final double finalTotal = totalOutcome;
                mainHandler.post(() ->
    ...
    private boolean processResultSet(ResultSet rs) throws Exception {
        ...
        while (rs.next()) {
            ...
            totalOutcome += rs.getDouble("outcome_cost");
        }
    ...
    private TableRow createTableRow(ResultSet rs, int index) throws Exception {
        ...
        row.addView(createCell(String.valueOf(rs.getInt("outcome_id")), 70));
        row.addView(createCell(rs.getString("outcome_date"), 0));
        row.addView(createCell(rs.getString("outcome_details"), 350));

        TextView costCell = createCell(String.format(Locale.US, "%.2f", rs.getDouble("outcome_cost")), 0);
        costCell.setBackgroundColor(0xFFFFEBEE); // Light red background for expenses
        row.addView(costCell);

        row.addView(createCell(String.valueOf(rs.getInt("outcome_fator_id")), 0));
        row.addView(createCell(rs.getString("outcome_source"), 0));
        row.addView(createCell(rs.getString("outcome_user"), 90));
        row.addView(createCell(rs.getString("outcome_far3"), 120));
        row.addView(createCell(rs.getString("outcome_notes"), 130));
